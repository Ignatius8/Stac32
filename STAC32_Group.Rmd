...
Next, we analyze the effects of the size of bill have on on tip percentage. We decided to split the bill amount into 3 groups based on quartile one and quartile three. Q1 are bill amount below Q1, Q2 are amounts between Q1 and Q3, Q3 are amounts equal to or beyondQ3.
...
```{r}
tip %>% mutate(Quartile = ifelse(Bill <=15.19, "Q1", ifelse(Bill <20.22, "Q2", "Q3"))) -> tipbill
```
...
Below are the quantile plots for Q1, Q2 and Q3. It doesn't appear normal so we will do a median test here.
...
The results from he pair wise median test shows that the Q1-Q3 is significant.